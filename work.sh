...
BASE_BUILD_DIR="build"              # 'rtl66/build'
...
show_version () {
   echo "Cfg66 version $RTL66_LIBRARY_VERSION $RTL66_SCRIPT_EDIT_DATE"
}
...
install_project () {
   ...
   if test "$USERID" = 0 ; then
      ...
      echo "UID $USERID. We want you as root to install the $RTL66 library..."
   fi
}
...
uninstall_project () {
   USERID=$(id -u)
   if test "$USERID" = 0 ; then
      echo "Uninstalling the $RTL66 library..."
      ninja -C $BUILD_DIR uninstall
      if test "$PLATFORM" = "UNIX" ; then
         rm -rf "$INSTALL_PREFIX/include/$RTL66_LIBRARY"
         rm -rf "$INSTALL_PREFIX/$INSTALL_LIBDIR/$POTEXT_LIBRARY"
         rm -rf "$INSTALL_PREFIX/share/doc/$RTL66"
#        rm -rf "$INSTALL_PREFIX/man/man1/$RTL66.1"
      fi
   else
      echo "UID $USERID. We want you as root to uninstall the $RTL66 library..."
   fi
}
