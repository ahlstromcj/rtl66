 ...
 *  <data bytes>, which is a space-separated list of byte-strings in
 *  hex format.
 */

bool
midimacros::add (const lib66::tokenization & tokens)
{
...
/**
 *  Converts all the loaded macros into bytes, expanding macro
...
midimacros::expand (midimacro & m)
{
    ...
    midi::bytes temp;                     /* holds bytes of 1 event if ! N/A  */
    ...
    for (const auto & token : m.tokens())
    {
        if (token[0] == '$')
        {
            ...
            {
                /*
                 * Was a bytestring: result += expand(cit->second);
                 */

                midi::bytes xpanded { expand(cit->second) };
