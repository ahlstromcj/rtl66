...
bool
sequence::first_notes (midi::pulse & ts, int & n) const
{
    xpc::automutex locker(m_mutex);
 ...
 *  from toggling the mute status?  The midi::pulse and bool parameters of the
...
bool
sequence::toggle_playing (midi::pulse tick, bool resumenoteons)
{
    set_armed(! armed());
...
/**
 *  Links a new event.  Locked elsewhere, no need for xpc::automutex
 *  locker(m_mutex).
 */
...
{
    if (is_good_channel(midi::byte(channel)))
    {
        xpc::automutex locker(m_mutex);
...
bool
sequence::move_selected_notes (midi::pulse delta_tick, int delta_note)
{
    xpc::automutex locker(m_mutex);
    m_events_undo.push(m_events);                  /* push_undo(), no lock */
...
bool
sequence::move_selected_events (midi::pulse delta_tick)
{
    xpc::automutex locker(m_mutex);
    m_events_undo.push(m_events);                  /* push_undo(), no lock */
...
bool
sequence::stretch_selected (midi::pulse delta_tick)
{
    xpc::automutex locker(m_mutex);
    m_events_undo.push(m_events);           /* push_undo(), no lock  */
...
bool
sequence::grow_selected (midi::pulse delta)
{
        ...
        if (er.is_selected_status(astatus))
        {
            ...
            else
            {
                ...
                if (datitem > (c_midi::byte_data_max - 1))
                    datitem = (c_midi::byte_data_max - 1);
...
sequence::repitch (const notemapper & nmap, bool all)
{
    ...
    for (auto & e : m_events)
    {
        ...
        {
            midi::byte pitch, velocity;                           /* d0 & d1  */
            e.get_data(pitch, velocity);
            pitch = midi::byte(nmap.convert(int(pitch)));
...
(
    ...
    waveform w, midi::byte status, midi::byte cc, bool usemeasure
)
{
    xpc::automutex locker(m_mutex);
    bool modified = false;
 ...
 *      c_null_midi::pulse, which means just detect the first time-signature
...
(
    ...
    midi::byte d0, midi::byte d1, bool repaint
)
{
    xpc::automutex locker(m_mutex);
    bool result = tick >= 0;
...
(
    midi::pulse tick, midi::pulse len, midi::pulse offset,
    midi::byte tpose, bool fixoffset
)
{
    xpc::automutex locker(m_mutex);
...
bool
sequence::intersect_triggers (midi::pulse position)
{
    xpc::automutex locker(m_mutex);
...
bool
sequence::grow_trigger (midi::pulse tickfrom, midi::pulse tickto)
{
    xpc::automutex locker(m_mutex);
...
const trigger &
sequence::find_trigger (midi::pulse tick) const
{
    xpc::automutex locker(m_mutex);
...
void
sequence::set_trigger_offset (midi::pulse trigger_offset)
{
    xpc::automutex locker(m_mutex);
    if (get_length() > 0)
    {
...
void
sequence::adjust_trigger_offsets_to_length (midi::pulse newlength)
{
    xpc::automutex locker(m_mutex);
...
void
sequence::copy_triggers (midi::pulse starttick, midi::pulse distance)
{
    xpc::automutex locker(m_mutex);
...
sequence::move_triggers
(
    midi::pulse tick, bool adjustoffset, triggers::grow which
)
{
    xpc::automutex locker(m_mutex);
...
void
sequence::offset_triggers (midi::pulse tick, triggers::grow editmode)
{
    xpc::automutex locker(m_mutex);
...
bool
sequence::get_trigger_state (midi::pulse tick) const
{
    xpc::automutex locker(m_mutex);
...
bool
sequence::select_trigger (midi::pulse tick)
{
    xpc::automutex locker(m_mutex);
...
bool
sequence::unselect_trigger (midi::pulse tick)
{
    xpc::automutex locker(m_mutex);
...
bool
sequence::paste_trigger (midi::pulse paste_tick)
{
    xpc::automutex locker(m_mutex);
 ...
 * \param range
 *      Provides how far to look for a time signature. The default is
 *      c_null_midi::pulse, which means just detect the first Meta event no
...
void
sequence::set_last_tick (midi::pulse tick)
{
    xpc::automutex locker(m_mutex);
    if (is_null_midi::pulse(tick))
        tick = m_length;
...
sequence::set_length (midi::pulse len, bool adjust_triggers, bool verify)
{
    ...
    if (result)
    {
        ...
        if (len > 0)
        {
            if (len < midi::pulse(m_ppqn / 4))
                len = midi::pulse(m_ppqn / 4);
...
{
    xpc::automutex locker(m_mutex);
    m_step_edit_note_length = midi::pulse(len);
}
...
void
sequence::shift_notes (midi::pulse ticks)
{
    xpc::automutex locker(m_mutex);
    if (get_length() > 0)
    {
...
bool
sequence::quantize_events (midi::byte status, midi::byte cc, int divide)
{
    xpc::automutex locker(m_mutex);
    if (divide == 0)
        return false;
...
bool
sequence::push_quantize (midi::byte status, midi::byte cc, int divide)
{
    xpc::automutex locker(m_mutex);
    m_events_undo.push(m_events);
...
void
sequence::play_queue (midi::pulse tick, bool playbackmode, bool resumenoteons)
{
...
void
sequence::song_recording_start (midi::pulse tick, bool snap)
{
    song_recording(true);
...
void
sequence::song_recording_stop (midi::pulse tick)
{
    (void) perf()->calculate_snap(tick);      /* issue #44 redux  */
...
void
sequence::resume_note_ons (midi::pulse tick)
{
    xpc::automutex locker(m_mutex);                     /* better here?     */
    if (get_length() > 0)
    {
