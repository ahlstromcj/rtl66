...
setmapper::max_timestamp () const
{
    ...
    {
        midi::pulse t = sset.second.max_timestamp();
        if (t > result)
            result = t;
    }
    return result;
...
setmapper::max_extent () const
{
    ...
    for (const auto & sset : sets())
    {
        midi::pulse t = sset.second.max_extent();
        if (t > result)
            result = t;
    }
    return result;
...
setmapper::max_trigger () const
{
    ...
    for (const auto & sset : sets())
    {
        midi::pulse t = sset.second.max_trigger();
        if (t > result)
            result = t;
    }
    return result;
...
)
{
    ...
    {
        midi::pulse distance = righttick - lefttick;
        if (seqno == seq::all())
        {
            for (auto & sset : sets())     /* screenset reference  */
            {
...
(
    midi::pulse lefttick, midi::pulse righttick, seq::number seqno
)
{
    if (righttick > lefttick)
    {
