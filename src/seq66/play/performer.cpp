 ...
 *  This class is probably the single most important class in Seq66v2, as it
...
bool
performer::is_input_system_port (midi::bussbyte bus) const
{
    return master_bus() ?
 ...
 *      midi::clocking_off, midi::clocking_pos, midi::clocking_mod, or (new) midi::clocking_disabled.
 */
...
std::string
performer::pulses_to_measure_string (midi::pulse tick) const
{
...
std::string
performer::pulses_to_time_string (midi::pulse tick) const
{
...
void
performer::repitch (midi::event & ev) const
{
...
void
performer::set_tick (midi::pulse tick, bool dontreset)
{
    if (tick >= 0)
    {
        m_tick = tick;
...
void
performer::set_left_tick (midi::pulse tick)
{
    m_left_tick = tick;
    set_start_tick(tick);
    m_reposition = false;
...
void
performer::set_right_tick (midi::pulse tick)
{
    if (tick == 0)
        tick = m_one_measure;
...
performer::set_midi_channel (seq::number seqno, int channel)
{
    ...
    if (result)
    {
        ...
        result = s->set_midi_channel(midi::byte(channel), true);  /* user ch. */
    }
    return result;
}
...
void
performer::jack_reposition (midi::pulse tick, midi::pulse stoptick)
{
...
performer::output_func ()
{
    ...
    while (! done())                        /* the variable is atomic       */
    {
        ...
        while (is_running())
        {
            ...
            {
                m_master_bus->init_clock(midi::pulse(pad().js_clock_tick));
            ...
            if (pad().js_dumping)
            {
                if (looping())
                {
                    ...
                    midi::pulse rtick = get_right_tick();     /* can change? */
                    if (pad().js_current_tick >= rtick)
                    {
                        ...
                        midi::pulse ltick = get_left_tick();
                ...
                m_master_bus->emit_clock(midi::pulse(pad().js_clock_tick));
            }
        ...
        if (! m_dont_reset_ticks)
        {
            midi::pulse start = song_mode() ? get_left_tick() : 0 ;
            if (is_jack_master())
...
void
performer::midi_song_pos (const midi::event & ev)
{
    midi::byte d0, d1;
    ev.get_data(d0, d1);
...
void
performer::midi_sysex (const midi::event & ev)
{
    if (rc().show_midi())
        ev.print();
...
bool
performer::auto_play_stop (midi::pulse tick)
{
    bool result = false;
...
void
performer::play (midi::pulse tick)
{
...
void
performer::play_all_sets (midi::pulse tick)
{
    if (tick > get_tick() || tick == 0)                 /* avoid replays    */
    {
        set_tick(tick);
...
void
performer::box_insert (seq::number dropseq, midi::pulse droptick)
{
...
void
performer::box_delete (seq::number dropseq, midi::pulse droptick)
{
    seq::pointer s = get_sequence(dropseq);
    if (s)
    {
...
void
performer::box_toggle_sequence (seq::number dropseq, midi::pulse droptick)
{
...
void
performer::box_move_triggers (midi::pulse tick)
{
    for (auto s : m_selected_seqs)
    {
...
void
performer::box_move_triggers (midi::pulse offset)
{
    for (auto s : m_selected_seqs)
    {
        seq::pointer selseq = get_sequence(s);
...
{
    midi::pulse timelen = get_max_timestamp();
    midi::pulse triglen = get_max_trigger();
    midi::pulse result = set_mapper().max_extent();
    if (triglen > result)
        result = triglen;
...
{
    midi::pulse tick = get_max_extent();
...
bool
performer::select_trigger (seq::number dropseq, midi::pulse droptick)
{
    seq::pointer s = get_sequence(dropseq);
    bool result = bool(s);
    if (result)
...
bool
performer::get_trigger_state (seq::number seqno, midi::pulse tick) const
{
    const seq::pointer s = get_sequence(seqno);
...
bool
performer::calculate_snap (midi::pulse & tick)
{
...
bool
performer::add_trigger (seq::number seqno, midi::pulse tick, midi::pulse snap)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
    if (s)
    {
        midi::pulse seqlength = s->get_length();
...
bool
performer::delete_trigger (seq::number seqno, midi::pulse tick)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
    if (s)
...
(
    seq::number seqno, midi::pulse tick, int transposition)
{
    bool result = false;
    if (transposition != 0)
    {
...
bool
performer::add_or_delete_trigger (seq::number seqno, midi::pulse tick)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
    if (s)
...
const trigger &
performer::find_trigger (seq::number seqno, midi::pulse tick) const
{
...
bool
performer::paste_trigger (seq::number seqno, midi::pulse tick)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
    if (s)
...
bool
performer::paste_or_split_trigger (seq::number seqno, midi::pulse tick)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
    if (s)
...
(
    seq::number seqno, midi::pulse tick, bool adjust_offset
)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
...
bool
performer::intersect_triggers (seq::number seqno, midi::pulse tick)
{
    bool result = false;
    seq::pointer s = get_sequence(seqno);
    if (s)
...
bool
performer::midi_control_event (const midi::event & ev, bool recording)
{
...
{
    midi::booleans original = get_mutes(gmute);
