...
metrosettings::calculate_length (int increment, float fraction)
{
    ...
    if (fraction > 0.1)             /* sanity float check   */
        result = midi::pulse(increment * fraction);
    else
        result = increment / 2;
...
metro::init_setup (performer * p, int measures)
{
    ...
    if (result)
    {
        ...
        midi::byte channel = settings().channel();    /* seq_midi_channel()   */
...
metro::initialize (performer * p)
{
    ...
    if (result)
    {
        ...
        for (int count = 0; count < bpb; ++count, tick += increment)
        {
            ...
            midi::event prog(tick, EVENT_PROGRAM_CHANGE | channel, patch);
            midi::event on(tick + 1, EVENT_NOTE_ON, channel, note, vel);
            midi::event off(tick + len, EVENT_NOTE_OFF, channel, note, vel);
...
recorder::initialize (performer * p)
{
    ...
    if (result)
    {
        ...
        if (settings().initialize(increment))
        {
            ...
            midi::bussbyte outbuss = settings().thru_buss();
            midi::byte channel = settings().thru_channel();
