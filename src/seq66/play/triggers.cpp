...
(
    ...
    midi::pulse offset, midi::byte tpose
) :
    m_tick_start    (tick),
...
int
triggers::datasize (midi::ulong sqspec) const
{
    return count() * trigger::datasize(sqspec);
}
...
midi::pulse
triggers::adjust_offset (midi::pulse offset)
{
    if (m_length > 0)
    {
        offset %= m_length;
...
(
    ...
    midi::byte transpose, bool fixoffset
)
{
    if (tick >= 0 && len >= 0)
    {
...
bool
triggers::intersect (midi::pulse position, midi::pulse & start, midi::pulse & ender)
{
...
bool
triggers::intersect (midi::pulse position)
{
    for (auto & ti : m_triggers)
    {
        if (ti.tick_start() <= position && position <= ti.tick_end())
...
void
triggers::adjust_offsets_to_length (midi::pulse newlength)
{
    for (auto & t : m_triggers)
    {
...
const trigger &
triggers::find_trigger (midi::pulse tick) const
{
...
void
triggers::offset_selected (midi::pulse tick, grow editmode)
{
    for (auto & t : m_triggers)
    {
        if (t.selected())
...
void
triggers::paste (midi::pulse paste_tick)
{
    if (m_trigger_copied)
    {
 ...
 *  appropriate.  Don't confuse this function with select(midi::pulse).
 *
 * \param t
 ...
 *  appropriate.  Don't confuse this function with unselect(midi::pulse).
 *
 * \param t
 *      Provides a reference to the desired trigger.
 *
