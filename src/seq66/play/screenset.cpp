...
void
screenset::song_recording_start (midi::pulse tick, bool snap)
{
    for (auto & s : m_container)
    {
        if (s.active())
...
void
screenset::song_recording_stop (midi::pulse tick)
{
    for (auto & s : m_container)
    {
        if (s.active())
...
void
screenset::set_last_ticks (midi::pulse tick)
{
    for (auto & s : m_container)
    {
        if (s.active())
    ...
    if (result)
    {
        ...
        for (seq::number s = offset(); s != m_set_maximum; ++s, ++bit)
        {
            ...
            if (sp)
            {
                ...
                bits.push_back(midi::boolean(false));
        }
    }
    return result;
}
