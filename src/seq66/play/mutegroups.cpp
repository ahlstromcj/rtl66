 ...
 *  performer.
 *
 *  Do not confuse it with the mutegroupmanager, which supports the
 *  performer's processing of mute-group selections.
 */
...
bool
mutegroups::load (mutegroup::number gmute, const midi::booleans & bits)
{
    bool result = gmute >= 0;
    if (result)
    {
    ...
    if (result)
    {
        ...
        if (result)
        {
            ...
            for ( ; mit != mutebits.end(); ++mit, ++ait)
            {
                ...
                {
                    if (*mit)                           /* on in group?     */
                        *ait = midi::boolean(false);         /* force it off     */
                }
                else
                {
                    *ait = *mit || *ait;
