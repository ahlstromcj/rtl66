...
songsummary::write (performer & p, bool doseqspec)
{
    ...
    if (result)
    {
        for (int trk = 0; trk < p.sequence_high(); ++trk)
        {
            if (p.is_seq_active(trk))
            {
                seq::pointer s = p.get_sequence(trk);
                if (s)
                {
...
)
{
    ...
    std::map<midi::ulong, std::string>::const_iterator ci =
