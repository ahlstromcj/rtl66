...
#include "midi/scales.hpp"              /* midi::scales declarations        */
#include "util/strfunctions.hpp"        /* util::contains()                 */

#if defined RTL66_USE_SHOW_ALL_COUNTS
#include "cfg/settings.hpp"             /* rtl66::rc().verbose()            */
#endif
...
double
midi_note_frequency (midi::byte note)
{
    double result = 0.0;
    if (note < c_byte_data_max)
    {
...
static bool
analyze_note (midi::byte note, keys & outkey, int & outoctave)
{
    bool result = note < c_byte_data_max;
    if (result)
    {
...
{
    bool hasminor = util::contains(keysigname, "min");
    bool hasmajor = util::contains(keysigname, "maj");
    ...
    if (result)
    {
        ...
        if (sfcount != (-99))
        {
            keysigbytes.push_back(midi::byte(sfcount));
            keysigbytes.push_back(hasminor ? midi::byte(1) : 0);
        }
        else
            result = false;
    }
