    ...
    for (auto & er : m_events)
    {
        if (er.is_selected())
        {
            ...
            if (canselect)
            {
                ...
                if (er.is_note_on_linked() && fixlink)
                {
                    /*
        ...
        for (auto & ev : m_events)
        {
            ...
            if (ev.is_note_on())
            {
                ...
                if (linked)
                {
                    ...
                    {
                        offstamp = midi::pulse(offstamp * factor);
...
eventlist::remove_first_match (const event & e, midi::pulse starttick)
{
    ...
    for (auto i = m_events.begin(); i != m_events.end(); ++i)
    {
        ...
        if (t >= starttick)
        {
            ...
            {
                (void) remove(i);           /* iterator required here       */
                result = true;
                break;
            }
        }
...
eventlist::rescale (midi::ppqn newppqn, midi::ppqn oldppqn)
{
    ...
    if (result)
    {
        ...
        length(rescale_tick(length(), newppqn, oldppqn));
    }
    return result;
}
...
midi::pulse
eventlist::trim_timestamp (midi::pulse t) const
{
    if (t >= length())
        t -= length();

    if (t < 0)                          /* only if midi::pulse is signed  */
        t += length();

    if (t == 0)
        t = length() - note_off_margin();
...
midi::pulse
eventlist::clip_timestamp (midi::pulse ontime, midi::pulse offtime, int snap) const
{
    if (offtime <= ontime)
        offtime = ontime + snap - note_off_margin();
    else if (offtime >= length())
        offtime = length() - note_off_margin();
