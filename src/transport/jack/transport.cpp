...
/**
 * \file          transport/jack/transport.cpp
 *
 ...
 * NSM Support:
 *
 *      Seq66 supports NSM; see the code in librtl66/ * /sessions and in
 ...
 * rc() values:
 *
 *      -   investigate(), investigate_disabled()
 *      -   jack_session() [uuid]
 *      -   with_jack())
 *      -   with_jack_transport()
 *      -   with_jack_master()
 *      -   with_jack_master_cond()
 *      -   jack_session_activate()
 *      -   app_client_name()
 *      -   home_config_directory()
 *      -   midi_filepath() (JACK session?)
 *
 * performer/player parent() values:
 *
 *      -   is_running()
 *      -   beats_per_minute()
 *      -   beats_per_minute()
 ...
 *      -   right_tick()
 *      -   left_right_size()
 *      -   signal_save(), signal_quit() (JACK session)
 *      -   off_sequences()/off_tracks()
 */

#include "rtl/rtl_build_macros.h"       /* RTL66_EXPORT, etc.               */

#if defined RTL66_JACK_TRANSPORT
...
#define RTL66_BPMINUTE_CALCULATION      /* portfix branch 2022-02-11        */

#if defined RTL66_JACK_SESSION          /* deprecated, use Non Session Mgr. */

#if defined RTL66_HAVE_JACK_METADATA_H
...
#include "midi/file.hpp"                /* midi::file class                 */
...
/*
 * -------------------------------------------------------------------------
 *  Static JACK Transport Functions
 * -------------------------------------------------------------------------
 */
...
transport::parameters transport::sm_jack_parameters;

bool
transport::save_jack_parameters
(
...
)
{
    ...
    if (result)
    {
        ...
        util::async_safe_errprint("JACK transport changed");
    }
    return result;
}

const transport::parameters &
transport::get_jack_parameters ()
{
 ...
 *  const accessors to jack::transport so that outsiders can monitor some of
 *  its status.
 *
 ...
 *
 * \param jack
 *      The jack::transport object for which to show debugging data.
 *
 * \param current_tick
 ...
 *
 * \param arg
 *      Provides the jack::transport pointer.
 *
 * \return
 ...
 *  project, to allow rtl66 to follow JACK transport.  For more advanced
 ...
 *
 *  Issue #34 "rtl66 doesn't follow jack::transport tempo changes": we no
 *  longer make this conditional upon player not running.
 *
            ...
            if (j->is_slave())
            {
                if (pos.beats_per_minute > 1.0)         /* a sanity check   */
                {
                    ...
                    if (pos.beats_per_minute != s_old_bpm)
                    {
                        ...
                        j->parent().beats_per_minute(pos.beats_per_minute);
                    }
                }
            }
...
\verbatim
          Possible data race during read of size 4 at 0xF854E58 by thread #1
             by 0x267602: rtl66::create_jack_client(...)
 ...
 *      jack_client_open().  By default, this name is the macro RTL66_PACKAGE
 *      (i.e.  "rtl66").  The name scope is local to each server. Unless
...
create_jack_client (std::string clientname, std::string uuid)
{
    ...
    else
    {
        ...
        if (not_nullptr(result))    //  && rc().investigate())
        {
            ...
            (void) util::info_message(t);
        }
    }
    if (not_nullptr(result))
    {
        if (status & JackServerStarted)
            (void) util::info_message("JACK server started");
        else
            (void) util::info_message("JACK server already started");
        ...
        (void) util::error_message("JACK server not running");

    return result;                      /* bad result handled by caller     */
}

/**
 *  A free function in the rtl namespace
 *
...
#if defined RTL66_JACK_METADATA && defined RTL66_HAVE_JACK_METADATA_H
...
#endif  // defined RTL66_JACK_METADATA && defined RTL66_HAVE_JACK_METADATA_H
...
show_jack_statuses (unsigned bits)
{
    ...
    {
        if (bits & jsp->jf_bit)
            (void) util::info_message(jsp->jf_meaning);
 ...
 *  of them public!
 *
 *  Note that the player object currently calls jack::transport::init(), but
 *  that call could be made here instead.
 *
 * \param parent
 *      Provides a reference to the main player object that needs to
 *      control JACK event.
 *
 * \param bpminute
 ...
 *  rtl66 are not slow-sync clients.  We don't really need to be a
 *  slow-sync client, as far as we can tell.  We can't get JACK working
 *  exactly the way it does in rtl66 without the callback in place.  Plus, it
 ...
 *
 *      Implemented first patch from freddix/rtl66 GitHub project, to fix JACK
 ...
 *      prevent rtl66 freeze) if rtl66 is set as JACK Master, or if another
 *      client application, such as Qtractor, is running as JACK Master (and
 *      then rtl66 will apparently follow it).
 *
 * Stazed:
 *
 ...
 *      sense, since rtl66/32/64 are not "slow-sync" clients.
 *
 * \return
...
transport::init ()
{
    ...
    if (result)
    {
        ...
        {
            result = false;
            return util::error_message("No JACK server");
        }
        else
        {
            ...
            {
                result = false;
                return util::error_message("JACK set callback failed");
            }
        }
        ...
        if (result)
        {
...
#endif
            {
                ...
                if (jackcode == 0)
                {
                    (void) util::info_message("JACK transport master");
            ...
            if (! master_is_set)
            {
                ...
                (void) util::info_message("JACK transport slave");
            }
        }
        if (result)
        {
            ...
            if (result)
            {
                (void) util::info_message("JACK transport enabled");
                m_jack_running = true;
            }
            else
                (void) util::info_message("Running without JACK transport");
        }
    }
    return result;
}
    ...
    if (m_jack_running)
    {
        ...
        if (is_master())
        {
            ...
            if (::jack_release_timebase(m_jack_client) != 0)
                (void) util::error_message("Cannot release JACK timebase");
        }
        ...
        if (::jack_deactivate(m_jack_client) != 0)
            result = util::error_message("Can't deactivate JACK transport");

        if (::jack_client_close(m_jack_client) != 0)
            result = util::error_message("Can't close JACK transport");
    }
    return result;
}

/**
 *  Activate JACK here.  This function is called by player::activate() after
...
transport::activate ()
{
    ...
    if (not_nullptr(m_jack_client))
    {
        ...
        if (result)
        {
            (void) util::info_message("JACK activated");
        }
        else
        {
            m_timebase = timebase::none;
            (void) util::error_message("Can't activate JACK transport client");
        }
    }
    return result;
}
...
/**
 *  player::beats_per_minute() validates the BPM.  Also, since
...
void
transport::beats_per_minute (midi::bpm bpminute)
{
 ...
 *
 *  This position() function is called via player::position_jack() in the
 ...
 *      do we want to player this function if rc().with_jack_transport() is
 ...
 *      "current tick" value.  If it's value is bad (null_midi::pulse),
 *      then this parameter is set to 0 before being used.
 */

void
transport::position (bool songmode, midi::pulse tick)
{
#if defined RTL66_BUILD_JACK
    if (songmode)                               /* master in song mode  */
        tick = midi::is_null_pulse(tick) ? 0 : tick * c_jack_factor ;
    else
        tick = 0;
    ...
    if (is_master())
    {
        ...
        if (::jack_transport_locate(m_jack_client, jack_frame) != 0)
            (void) util::info_message("jack_transport_locate() failed");
    }
 ...
 *  player::position_jack() function.  We might be able to use it in other
 *  functions.
 *
...
void
transport::set_position (midi::pulse tick)
{
    jack_position_t pos;
 ...
 *  my computer, and allow rtl66 to follow another JACK Master, on some
 ...
 *  state parameter, then rtl66 also can follow another JACK Master.
 *  (CURRENTLY BUGGY!)
 *
...
transport::sync (jack_transport_state_t state)
{
    ...
    if (rate == 0)
    {
        ...
        errprint("jack::transport::sync(): zero frame rate");
        rate = 48000;
    }
    else
        result = 1;
...
#endif  // defined RTL66_JACK_SYNC_CALLBACK

#if defined RTL66_JACK_SYNC_CALLBACK

/**
 *  This JACK synchronization callback informs the specified player
 ...
 *
 * \param arg
 *      The pointer to the jack::transport object.  Currently not checked for
...
#endif  // RTL66_JACK_SYNC_CALLBACK

#if defined RTL66_JACK_SESSION          /* "deprecated" alternative to NSM  */
 ...
 * Command line:
 *
 *      qrtl66 --jack-session-uuid UUID --home SESSION_DIR
 *
 * Event types:
 *
...
#endif
    {
        util::info_message(session_event_name(ev));
        util::file_message("Session command", cmd);
        util::file_message("Session path", filepath);
    }
...
/**
 *  Glib is then used to connect in player::jack_session_event().  However,
 *  the player object's GUI-support interface is used instead of the
 *  following, so that the librtl66 library can be independent of a specific
 *  GUI framework:
 *
 *      Glib::signal_idle().
 *          connect(sigc::mem_fun(*jack, &jack::transport::session_event));
 *
 * \param ev
 *      The JACK event to be set.
 *
 * \param arg
 *      The pointer to the jack::transport object.  Currently not checked
 *      for nullity.
 */
...
#endif  // RTL66_JACK_SESSION

/**
 *  Performance output function for JACK, called by the player function
 *  of the same name.  This code comes from player::output_func() from seq24.
 *
 * \note
 ...
 *      player object and the jack::transport object.
 *
 * \return
...
bool
transport::output (scratchpad & pad)
{
    if (m_jack_running)
    {
        ...
        if (transport_rolling_now())
        {
            ...
            {
                if (pad.js_current_tick >= parent().right_tick())
                {
                    /*
                    ...
                    parent().off_tracks();
                    parent().set_last_ticks(midi::pulse(pad.js_current_tick));
                }
            }
        }
        ...
        if (pad.js_dumping)
        {
            ...
            if (m_frame_current > m_frame_last)         /* moving ahead?    */
            {
                ...
                if (jack_pos().frame_rate > 0)          /* usually 48000    */
                {
                    ...
                    util::info_message("JACK output 2 zero frame rate");
...
void
transport::update_timebase_master (jack_transport_state_t s)
{
 ...
 * \param name
 *      Provides the name of the client, used in the call to
 *      create_jack_client().  By default, this name is the macro RTL66_PACKAGE
 *      (i.e.  "rtl66").
 *
 * \return
 *      Returns a pointer to the JACK client if JACK has opened the client
...
jack_client_t *
transport::client_open (const std::string & name)
{
 ...
 *
 * \param arg
 *      Provides the putative jack::transport pointer, assumed to be not null.
 *
 * \return
...
long
transport::current_jack_position () const
{
    ...
    {
        util::error_message("Null JACK transport client");
        return 0;
    }
}
 ...
 *
 * \param arg
 *      Provides the jack::transport pointer, currently unchecked for nullity.
 */
...
/**
 *  This callback is to shut down JACK by clearing the jack::transport ::
 *  m_jack_running flag.
 *
 * \param arg
 *      Points to the jack::transport in charge of JACK support for the player
 *      object.
 */
...
jack_transport_shutdown (void * arg)
{
    ...
    {
        (void) util::error_message("null JACK transport pointer");
    }
}
...
#endif          // if defined RTL66_JACK_TRANSPORT
