...
\title{Rtl66 Developer Guide 0.2.0}
\author{Chris Ahlstrom \\
...
\label{sec:introduction}
   ...
   This document describes "Rtl66", an audio and MIDI library based on the
   \textsl{RtAudio} \cite{rtaudio} and
   \textsl{RtMidi} \cite{rtmidi} libraries, with changes and extensions.
   The Audio library is not at all ready for prime time.
   The MIDI library is complete in its design, but only JACK and
   ALSA are basically complete at this time.
...
\label{subsec:what_is_rtl66}
   ...
   \textsl{Seq66v2} application, as well as other audio/MIDI
   applications.
   It is based on the \textsl{RtMidi} \cite{rtmidi} and
   \textsl{RtAudio} \cite{rtaudio} libraries,
   but with the following changes and additions:
   ...
   \begin{itemize}
      ...
      \item Additional support functions.  For example, a simple command-line
         parser and various test function have
         been added to support the test applications in a uniform way.
      ...
      \item Library code based on \textsl{Seq66}.
         ...
         There are more such modules to be completed in the future.
      \item Support for a "master bus".
         \textsl{RtMidi} supports each MIDI port having a separate client.
         The \textsl{Rtl66} master bus concept uses the same client for all
         ports, which has important implications for processing.
   \end{itemize}
   ...
   It also provides additional paradigms.
...
\label{subsec:history}
   ...
   simplify \textsl{Seq66}, it was decided to fold \textsl{Seq66} code
   into our refactored version of \textsl{RtMidi} as well.

\input{tex/high-level}
