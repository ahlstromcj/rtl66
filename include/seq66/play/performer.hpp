#if ! defined RTL66_PERFORMER_HPP
#define RTL66_PERFORMER_HPP
...
public:
    ...
    public:
        ...
        (
            int /* ppqn */, midi::bpm, performer::change
        )
        {
            return false;
        }
...
private:                            /* key, midi, and op container section  */
     ...
     *  with midi::bussbyte indices ranging from 0 to the number of input ports
     *  minus 1.
     */
...
public:
    ...
    (
        int ppq, midi::bpm bp, change mod = change::yes
    );
...
public:
    ...
    bool jack_set_beats_per_minute (midi::bpm bpm, bool user_change = false);
    ...
    void reposition (midi::pulse tick);
...
public:
    ...
    void box_insert (seq::number dropseq, midi::pulse droptick);
    void box_delete (seq::number dropseq, midi::pulse droptick);
    void box_toggle_sequence (seq::number dropseq, midi::pulse droptick);
    void box_unselect_sequences (seq::number dropseq);
    void box_move_triggers (midi::pulse tick);
    void box_move_triggers (midi::pulse offset);
    ...
    bool auto_play_stop (midi::pulse tick);
    void auto_stop (bool rewind = false);
    void auto_pause ();
    void auto_play ();
    void play_all_sets (midi::pulse tick);
    void play (midi::pulse tick);
    void all_notes_off ();
    ...
    void move_tick (midi::pulse tick, bool dontreset = false);
    void set_left_tick (midi::pulse tick);
    void set_left_tick_seq (midi::pulse tick, midi::pulse snap);
    ...
    midi::pulse get_left_tick () const
    {
        return m_left_tick;
    }

    void set_start_tick (midi::pulse tick)
    {
        m_start_tick = tick;         /* starting JACK tick/pulse value   */
    }

    midi::pulse get_start_tick () const
    {
        return m_start_tick;
    }

    void set_right_tick (midi::pulse tick);
    void set_right_tick_seq (midi::pulse tick, midi::pulse snap);

    midi::pulse get_right_tick () const
    {
        return m_right_tick;
    }
...
public:
    ...
    midi::bpm get_beats_per_minute () const
    {
...
public:
    ...
    midi::booleans get_mutes (mutegroup::number gmute) const
    {
        return mutes().get(gmute);
    }

    midi::booleans get_active_groups () const
    {
    ...
    midi::bpm decrement_beats_per_minute ();
    midi::bpm increment_beats_per_minute ();
    midi::bpm page_decrement_beats_per_minute ();
    midi::bpm page_increment_beats_per_minute ();
    ...
    std::string pulses_to_measure_string (midi::pulse tick) const;
    std::string pulses_to_time_string (midi::pulse tick) const;

    bool ui_set_input (midi::bussbyte bus, bool active);
    bool ui_get_input
    (
    ...
    midi::bussbyte true_input_bus (midi::bussbyte nominalbuss) const;
    midi::bussbyte true_output_bus (midi::bussbyte nominalbuss) const;
    ...
    void set_input (midi::bussbyte bus, bool inputing)
    {
    ...
    bool is_input_system_port (midi::bussbyte bus) const;
    bool new_ports_available () const;
    bool is_port_unavailable (midi::bussbyte bus, midibase::io iotype) const;
    ...
    bool select_trigger (seq::number seqno, midi::pulse droptick);
    bool selected_trigger
    (
        seq::number seqno, midi::pulse droptick,
        midi::pulse & tick0, midi::pulse & tick1
    );
    ...
    bool get_trigger_state (seq::number seqno, midi::pulse tick) const;
    bool add_trigger (seq::number seqno, midi::pulse tick, midi::pulse snap);
    bool delete_trigger (seq::number seqno, midi::pulse tick);
    bool transpose_trigger (seq::number, midi::pulse droptick, int tranposition);
    bool add_or_delete_trigger (seq::number seqno, midi::pulse tick);
    bool split_trigger
    (
        seq::number seqno,
        midi::pulse tick,
    ...
    (
        seq::number seqno,
        midi::pulse tickfrom, midi::pulse tickto,
        midi::pulse len
    );
    const trigger & find_trigger (seq::number seqno, midi::pulse tick) const;
    bool paste_trigger (seq::number seqno, midi::pulse tick = c_no_paste_trigger);
    bool paste_or_split_trigger (seq::number seqno, midi::pulse tick);

#if defined USE_INTERSECT_FUNCTIONS
    bool intersect_triggers (seq::number seqno, midi::pulse tick);
#endif

    bool offset_triggers
    (
        triggers::grow ts, int seqlow, int seqhigh, midi::pulse offset
    );
    bool move_triggers (seq::number seqno, midi::pulse tick, bool adjust_offset);
    bool move_trigger
    (
        seq::number seqno,
    ...
    midi::pulse record_snap_length () const
    {
...
public:         /* GUI-support functions */
    ...
    void record_snap_length (midi::pulse snap)
    {
    ...
    bool set_beats_per_minute (midi::bpm bp, bool user_change = false);
    bool set_ppqn (int p);
...
private:
    ...
    bool calculate_snap (midi::pulse & tick);
    void show_cpu ();
